...
export default function PositionEntry({ position, is_latest }: PositionEntryProps) {
    ...
    return (
        <div className="relative">
            <div
                className={`absolute -left-8 top-1.5 w-4 h-4 rounded-full border-2 border-charcoal-ink dark:border-pure-white
          ${is_latest ? "bg-on-background" : "bg-background"}`}
            ...
            <h4 className="font-serif text-lg font-semibold text-on-background mb-3">
                {position.role?.name}
            </h4>
